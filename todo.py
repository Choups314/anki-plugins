...
chapters = {}
# Un dictionnaire qui indique a quel chapitre appartient chaque note <NoteId, Chapitre>
notes = {}
...
####################################################################
def exeExtendDB():
	mw.col.db.execute("CREATE TABLE `todo` (`cardId` INTEGER, `chapitre`TEXT, `logicOrder` INTEGER, `done`INTEGER)")
	...
	for id,mid,flds in mw.col.db.execute("SELECT id,mid,flds FROM notes"):
		if mid in midFilter:
			...
			for cardId in mw.col.db.execute("SELECT cardId FROM todo WHERE `cardId`=%d" % id) :
				count += 1
			if count == 0:
				mw.col.db.execute("INSERT INTO `todo`(`cardId`,`chapitre`,`logicOrder`,`done`) VALUES (%d,'%s',0,0)" % (id, chapter))
...
def addLogicOrder(self):
	...
	if self.note.id in notes:
		...
		mw.col.db.execute("UPDATE todo SET `logicOrder`=%d WHERE `cardId`=%d" % (order, id))
...
def myAddNote(self, note):
	...
	if mid in midFilter:
		...
		mw.col.db.execute("INSERT INTO `todo`(`cardId`,`chapitre`,`logicOrder`,`done`) VALUES (%d,'%s',%d,0)" % (id, chapter, logicOrder))
	...
	for noteId in mw.col.db.execute("SELECT id FROM notes") :
		...
		while True:
			...
			for cardId in  mw.col.db.execute("SELECT id FROM cards WHERE `nid`=%d AND `ord`=%d" % (noteId[0], i)):
				if j>=1:
					mw.col.db.execute("DELETE FROM cards WHERE `id`=%d" % cardId)
					num += 1
				j += 1
			if j<=1:
				break
...
mw.connect(removeTODODuplicatesAction , SIGNAL("triggered()"), removeTODODuplicates)
